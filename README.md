# Rembrosoft Agent Hub

A **local** internal application that records and displays work performed by Claude Code and
Codex agents on the main Windows PC. Agents are controlled remotely through **Discord via
OpenACP**; during and after their work they report to the Agent Hub through the small
`agent-report` CLI.

**Division of responsibilities (V1):**

- **OpenACP** remains responsible for Discord communication and agent sessions.
- **Agent Hub records and displays work**, and acts as a **control panel for OpenACP**: it edits
  ...
  restart or stop the OpenACP daemon (see [OpenACP channel bindings](#openacp-channel-bindings)).
- **Agent Hub never starts or stops Claude Code or Codex directly** — agents are launched by
  OpenACP in response to Discord messages. Agent Hub also cannot restart its own backend or
  dashboard; use `stop-agent-hub.bat` / `start-agent-hub.bat` for that.
...
Task lifecycle: agents finish successful work as **`needs_review`** — only a human approval
in the dashboard moves a task to **`completed`**. Task IDs are sequential: `REM-001`, `REM-002`, …
...
The script verifies Python/Node, creates `backend\.venv`, installs backend + CLI + frontend
dependencies and copies `.env.example` to `.env` (existing `.env` is never overwritten).
...
Double-click **`start-agent-hub.bat`** in the repository root — it starts the backend, the
dashboard and the OpenACP daemon, waits for them, and opens the dashboard.

Anything already running is detected and left alone, so running it twice is safe (no
"port already in use" errors). Useful flags: `-DryRun` reports what it would start without
starting anything, `-NoBrowser` skips opening the dashboard.

To stop everything, double-click **`stop-agent-hub.bat`**. Closing the console windows does not
reliably terminate the processes they started — this kills them properly and reports the ports.
It only touches processes whose command line points at this repository or at the OpenACP CLI.
...
`restart-openacp.ps1` asks for confirmation first, because restarting terminates running
agent sessions; pass `-Force` to skip the prompt.
...
Relevant settings (all optional, see `.env.example`): `AGENT_HUB_OPENACP_SETTINGS_PATH`,
...
`AGENT_HUB_OPENACP_HOOK_TIMEOUT_SECONDS`.
...
- **Prerequisites (Python / Node).** These **cannot** be installed from the dashboard — it is
  ...
  See [Moving to another PC](#moving-to-another-pc).

## Moving to another PC

Copying the folder alone does **not** work: `backend\.venv` hard-codes absolute paths, `node_modules`
is platform-bound, and OpenACP + its workspace live outside the repository. The path is:

1. Copy the repository.
2. Double-click `install-agent-hub-prerequisites.bat` (installs Python, Node and PowerShell 7 via
   winget, then runs `setup.ps1` — recreates the venv, installs `node_modules`, creates `.env`).
3. **Before copying**, on the source PC's dashboard OpenACP page, click *Copy settings.json here* —
...
> the copied folder on purpose, but is gitignored so it cannot be committed. Keep it that way.

Settings: `AGENT_HUB_SCREENSHOT_DIR` (default `./screenshots`),
`AGENT_HUB_SCREENSHOT_INSTALL_TIMEOUT_SECONDS`, `AGENT_HUB_OPENACP_INSTALL_TIMEOUT_SECONDS`,
...
scripts/    setup.ps1, start-backend.ps1, start-frontend.ps1, start-all.ps1, stop-all.ps1,
            ...
            heartbeat-silent.vbs, cleanup-openacp-tunnels.ps1,
...
`@openacp/cli` and `@openacp/discord-adapter` are separate third-party packages
installed from npm; they are not part of this repository and carry their own licenses.
